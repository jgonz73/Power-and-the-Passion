               ...
               column(5,
                  ...
                  fluidRow(
                    ...
                    box(title = "Total Energy USMAP", status="primary", width=14,
                        plotOutput("map1", height=270)
                    )
                  ),
                  fluidRow(
                    #USMAP %
                    box(title = "% Total Energy USMAP", status="primary", width=14,
                        plotOutput("map1P", height=270)
                    )
                  )
               )
             )
             ...
             fluidRow(
               ...
               column(5,
                  ...
                  fluidRow(
                    #USMAP AMOUNT
                    box(title = "Total Energy USMAP", status="primary", width=14,
                        plotOutput("map2", height=270)
                    )
                  ),
                  fluidRow(
                    #USMAP%
                    box(title = "% Total Energy USMAP", status="primary", width=14,
                        plotOutput("map2P", height=270)
                    )
                  )
               )
             )
...
output$map1P <- renderPlot({
  plot_usmap(
    data=mapReactive1P(), values = "PERCENT", color="black"
...
output$map2P <- renderPlot({
  plot_usmap(
    data=mapReactive2P(), values = "PERCENT", color="black"
  ) + scale_fill_distiller(palette = "GnBu") + labs(fill = "% Generation MWh") + 
    theme_map() + theme(legend.position="right")
